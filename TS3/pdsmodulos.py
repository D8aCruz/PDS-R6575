...
def SignalGenerator(AA, ff, dc, N, fs, signal = 'Senoidal', ph = 0, duty = 0.5, width = 0.5, snr = None, noise = 'normal'):
    if signal == 'Senoidal':
        tt, xx = mi_funcion_sen(vmax=AA, dc=dc, ff=ff, ph=ph, nn=N, fs=fs)
        if snr == None:
            return tt, xx
        else:
